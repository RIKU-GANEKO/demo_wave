...
//	private static final Logger logger = Logger.getLogger(ParticipationCreateService.class.getSimpleName());
	...
	/**
	 * {@link ParticipationContext}を使用してユーザー作成のレスポンスを返すメソッドです。
	 *
	 ...
	 * </ol>
	 *
	 * @param participationContext
	 * @return ユーザー追加に関するレスポンス
	 */
	ResponseEntity<APIResponse> postParticipation(ParticipationContext participationContext) {
		participationContext.setParticipationDBLogic(participationDBLogic);
		try {
			return participationContext.postParticipation();
		}
		catch (Exception e) { // 内部的なエラーが起きた場合
			return participationContext.errorResponse(ErrorCode.INTERNAL_SERVER_ERROR.getCode(), ErrorCode.INTERNAL_SERVER_ERROR.getDescription(),
