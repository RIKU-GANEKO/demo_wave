...
public class EmailUserCreateDBLogic {
	 ...
	 */
	@CustomRetry
	boolean isExistingUser(java.util.UUID userId) {
	...
	User saveUser(String supabaseUid, String email, EmailUserRequest request) {
		java.util.UUID userId = java.util.UUID.fromString(supabaseUid);
	 ...
	 */
	private User mergeUser(User existingUser, String email, EmailUserRequest request) {
		// emailは常に最新に更新
