 ...
 */
@Builder
public class DemoParticipationRankingContext {
...
//	private static final Logger logger = Logger.getLogger(DemoListContext.class.getSimpleName());
	...
	@Setter
	private DemoParticipationRankingDBLogic demoParticipationRankingDBLogic;
	...
	ResponseEntity<APIResponse> getDemoList() {
		List<DemoParticipationRankingRecord> responses = null;
		...
		responses = demoParticipationRankingDBLogic.fetchDemoList(startDateTime, endDateTime);

		return new ResponseEntity<>(new DemoParticipationRankingResponse(responses), HttpStatus.OK);
	}
