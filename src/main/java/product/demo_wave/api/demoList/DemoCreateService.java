...
//	private static final Logger logger = Logger.getLogger(DemoListService.class.getSimpleName());
	 ...
	 * </ol>
	 *
	 * @param demoCreateContext
	 * @return デモ一覧情報に関するレスポンス
	 */
	ResponseEntity<APIResponse> postDemo(DemoCreateContext demoCreateContext) {
		demoCreateContext.setDemoCreateDBLogic(demoCreateDBLogic);
		try {
			return demoCreateContext.postDemo();
		}
		catch (Exception e) { // Token認証は突破したが内部的なエラーが起きた場合
			return demoCreateContext.errorResponse(ErrorCode.INTERNAL_SERVER_ERROR.getCode(), ErrorCode.INTERNAL_SERVER_ERROR.getDescription(),
